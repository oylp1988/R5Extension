...
import * as data from './resource/resource.json';

// Defines the search path of your language server DLL. (.NET Core)
const languageServerPaths = [
    "server/DemoLanguageServer.dll",
    "../../DemoLanguageServer/bin/Debug/net6.0/DemoLanguageServer.dll",
]
...
async function activateLanguageServer(context: vscode.ExtensionContext) {
    ...
    client = new LanguageClient(data.langServerNameId, data.langServerName, serverOptions, clientOptions);
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    let id = "extensions.riscv.sayHello"
    let disposable = vscode.commands.registerCommand(id, () => {
        // The code you place here will be executed every time your command is executed

        // Display a message box to the user
        vscode.window.showInformationMessage(data.greeting);
    });
